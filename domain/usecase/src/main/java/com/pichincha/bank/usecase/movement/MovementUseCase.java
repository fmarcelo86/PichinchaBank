...
public class MovementUseCase {
    ...
    public Mono<Movement> save(Movement movement) {
        ...
        return accountRepository.findByAccountNumber(movement.getAccountNumber())
                ...
                .filter(account -> movement.getAmount() != 0)
                .switchIfEmpty(Mono.error(new BusinessException(BusinessErrorMessage.AMOUNT_INVALID)))
                .filter(account -> account.getInitialBalance() + movement.getAmount() >= 0)
